...
// the list of songs:
var songs = ['01.mp3', '02.mp3', '03.mp3', '04.mp3'];
//var songs;
...
function preload() {          // load the first song on preload
 song = loadSound('framework/js/audio/' + songs[currentSong]);
 //song = loadSound('01.mp3')
// robot = loadSound('framework/js/audio/04.mp3');
}
...
function getSong(songNumber) {
  if (songNumber < songs.length) {   // if the song number is in range
    ...
    // load a new song:
    song = loadSound('framework/js/audio/'+ songs[currentSong], resumePlay);
    return true;
